    ...
    const oldestToYoungest = inventors.sort(function(firstPerson, secondPerson){
    ...
    // or using a ternary
    // const oldestToYoungest = inventors.sort((firstPerson, secondPerson) => firstPerson.year > secondPerson.year ? 1 : -1)
    ...
    const alphabetically = people.sort((lastPerson, firstPerson) => {
    ...
    const transporation = data.reduce((object, item) => {
        if(!object[item]){
            object[item] = 0;
        }
        object[item]++;
